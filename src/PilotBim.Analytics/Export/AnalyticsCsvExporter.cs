...
namespace PilotBim.Analytics.Export
{
    ...
    internal sealed class AnalyticsCsvExporter : IAnalyticsCsvExporter
    {
        public string Export(ProjectAnalyticsSnapshot snapshot)
        {
            ...
            WriteRows(Path.Combine(folder, "responsible.csv"),
                new[] { "DisplayName", "PersonId", "OrgUnitId", "OrganisationName", "SampledCount", "SharePercent", "Scope" },
                snapshot.ObjectsByResponsible,
                r => new[] { r.DisplayName, r.PersonId.HasValue ? r.PersonId.Value.ToString() : "", r.OrgUnitId.ToString(), r.OrganisationName ?? "", r.SampledCount.ToString(), F(r.SharePercent), r.Scope });
            ...
            WriteRows(Path.Combine(folder, "state_semantics.csv"),
                ...
                r => new[] { r.StateTitle, r.StateId.ToString(), r.Count.ToString(), F(r.SharePercent), r.Semantic, r.Scope });
                        ...
                        WriteRows(Path.Combine(folder, "scan_compare.csv"),
                new[] { "Area", "Metric", "Previous", "Current", "Delta", "Notes" },
                snapshot.ScanDiffRows,
                r => new[] { r.Area, r.Metric, r.Previous, r.Current, r.Delta, r.Notes });

            var summaryWithCompare = new List<AnalyticsKpiRow>();
            if (snapshot.Summary != null)
                summaryWithCompare.AddRange(snapshot.Summary);
            var diffCount = snapshot.ScanDiffRows != null ? snapshot.ScanDiffRows.Count : 0;
            summaryWithCompare.Add(new AnalyticsKpiRow
            {
                Label = "Scan compare rows",
                Value = diffCount.ToString(),
                Detail = diffCount == 0
                    ? "no diff (first scan or empty)"
                    : "see scan_compare.csv; LocalAppData Snapshots\\last-scan.json"
            });
            var changed = snapshot.ScanDiffRows == null
                ? 0
                : snapshot.ScanDiffRows.Count(r => r != null && r.Delta != null && r.Delta != "0" && r.Delta != "n/a");
            summaryWithCompare.Add(new AnalyticsKpiRow
            {
                Label = "Scan compare changed metrics",
                Value = changed.ToString(),
                Detail = "rows with numeric/time delta"
            });
            WriteKpi(Path.Combine(folder, "summary.csv"), summaryWithCompare);
