...
namespace PilotBim.Analytics.Services
{
    ...
    internal sealed class ProjectAnalyticsService : IProjectAnalyticsService
    {
        ...
        {
            var personMap = report.Persons.GroupBy(p => p.PersonId).ToDictionary(g => g.Key, g => g.First().DisplayName);
        ...
        {
            var orgMap = report.Organisations.ToDictionary(o => o.OrganisationId, o => o);
            var personMap = report.Persons.GroupBy(p => p.PersonId).ToDictionary(g => g.Key, g => g.First().DisplayName);
            var personByMainOrg = report.Persons
                .Where(p => p.OrganisationId.HasValue)
                .GroupBy(p => p.OrganisationId.Value)
            ...
            return report.ResponsibleSampleCounts
                ...
                .Select(pair =>
                {
                    ...
                    orgMap.TryGetValue(pair.Key, out org);
                    string orgName = org != null ? org.Name : null;

                    int? personId = null;
                    string personName = null;
                    if (org != null && org.PersonId.HasValue && org.PersonId.Value != 0)
                        personId = org.PersonId;
                    else
                    {
                        PersonInventoryRecord byMain;
                        if (personByMainOrg.TryGetValue(pair.Key, out byMain))
                            personId = byMain.PersonId;
                    }

                    if (personId.HasValue)
                    {
                        if (!personMap.TryGetValue(personId.Value, out personName) || string.IsNullOrWhiteSpace(personName))
                            personName = "PersonId=" + personId.Value;
                    }
                    ...
                    {
                        display = string.IsNullOrWhiteSpace(orgName)
                            ? personName
                            : personName + " (" + orgName + ")";
                        scope = "OrgUnit→Person (OrganisationUnitExtensions.Person)";
                    }
                    else if (!string.IsNullOrWhiteSpace(orgName))
                    {
                        display = orgName;
                        scope = "OrgUnit title only (person not linked)";
                    }
                    else
                    {
                        display = "OrgUnitId=" + pair.Key;
        ...
        private static List<string> BuildLimitations(ProjectInventoryReport report)
        {
            ...
            var list = new List<string>
            {
                ...
                "Сравнение сканов — last-scan.json + именованная история в %LOCALAPPDATA%\\PilotBim.Analytics\\Snapshots (не в Pilot).",
                "Layout сводки — %LOCALAPPDATA%\\PilotBim.Analytics\\dashboard-layout.json (вкл/выкл и порядок блоков).",
                creatorNote,
                "UserState OPEN/CLOSED — эвристика по названию; можно переопределить в %LOCALAPPDATA%\\PilotBim.Analytics\\state-mapping.json",
                "Ответственный — OrgUnit (должность) → Person.DisplayName через OrganisationUnitExtensions.Person; без связи показывается Title должности.",
