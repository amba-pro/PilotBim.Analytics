...
namespace PilotBim.Analytics.Services
{
    internal sealed class CapabilityMatrixService
    {
        public void Build(ProjectInventoryReport report)
        {
            ...
            list.Add(Metric("Объекты по ответственному", "OrgUnit/Person attribute", orgUnitAttrs ? (orgPersonLinked ? CapabilityStatus.Available : CapabilityStatus.Partial) : CapabilityStatus.NotVerified, orgUnitAttrs && orgPersonLinked ? CapabilityStatus.Ready : CapabilityStatus.NeedsMapping, "OrgUnit position → OrganisationUnitExtensions.Person → IPerson.DisplayName"));
            ...
            list.Add(Metric("Средний срок закрытия", "CreatedDate + ClosedDate/state transition history", CapabilityStatus.NotAvailable, CapabilityStatus.Blocked, "Blocked: no dedicated ClosedDate; no state-transition journal in SDK; HistoryItems empty on sampled objects"));
            list.Add(Metric("Время реакции / SLA", "State transition timestamps + author", CapabilityStatus.NotAvailable, CapabilityStatus.Blocked, "Blocked: ITransitionManager is config-only; no transition journal; runtime HistoryItems ids empty on sample"));
